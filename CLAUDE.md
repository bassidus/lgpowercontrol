...
- **What works: NM dispatcher `pre-down.d`** — runs blocking at sleep with network still up (plain-NIC setups), gated on logind's `PreparingForSleep` property. Shipped as `scripts/90-lgpowercontrol` (v2.3). Almost-true caveat (seen on p600s 2026-07-27, 1 of 10+): pre-down blocks NM's device-state transition but not its parallel DHCP-cancel/IP-flush, which can finish before the script spawns (~10 ms vs ~70 ms after PrepareForSleep) → power_off fails with Errno 101, link still up. Opt-in fix documented in README: enable NIC WoL (`802-3-ethernet.wake-on-lan magic`) so NM skips the device and the race-free sleep hook takes over. Default-enabling NIC WoL was considered and rejected.
